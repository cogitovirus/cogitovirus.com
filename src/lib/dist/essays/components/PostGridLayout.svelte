  ...
  {#each posts as post}
    <div>
      <a href="/{joinPath(blogDirName, post.slug)}" aria-label="post link to {post.title}">
        <article
          class="bg-base-200 hover:border-accent flex min-h-full flex-col rounded-xl border-2 border-transparent p-6"
        >
          <header>
            <h2 class="text-2xl leading-8 font-extrabold tracking-tight text-balance">
              {post.title}
            </h2>
          </header>
          ...
          <footer class="text-base-content/70 mt-auto text-sm leading-6 font-medium">
            {#if post.category}
              <span>
